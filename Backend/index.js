...
app.post('/sign-up', async (req, res) => {
    ...
    try {
        ...
        console.log(err);
        res.status(500).send({ success: false, message: "Some error happened in the registration", err: err });
    }
});
    ...
    try {
        ...
        if (loginData) {
            bcrypt.compare(req.body.userpassword, loginData.userpassword, function (err, result) {
                if (result == true) {
                    jwt.sign({ userId: loginData._id, useremail: loginData.useremail, username: loginData.username, userimage: loginData.userimage }, 'JOBPORTEL', { expiresIn: '168h' }, function (err, token) {
                        if (!err) {
                            jwt.verify(token, 'JOBPORTEL', function (err, decoded) {
                                if (!err) {
                                    res.send({ success: true, token, useremail: decoded.useremail, userId: decoded.userId, username: decoded.username, userimage: decoded.userimage })
                                }
                            });
                        }
                        else {
...
app.post('/google', async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            const token = jwt.sign({ id: existingUser._id, useremail: existingUser.useremail, username: existingUser.username, userimage: existingUser.userimage }, 'JOBPORTEL', { expiresIn: '168h' });
            const decoded = jwt.verify(token, 'JOBPORTEL');
            res.send({ success: true, token: token, userId: decoded.id, useremail: decoded.useremail, username: decoded.username, userimage: decoded.userimage });
        }
        else {
        ...
        res.status(500).send({ success: false, message: "Internal Server Error", err: error });
    }
});
...
app.get("/GetTheRoleInfo/:email", async (req, res) => {
    connectDatabase()
    try {
        const data = await onboardModel.findOne({ email: req.params.email })
        if(data){
            res.send({ data: data })
        }
    }
